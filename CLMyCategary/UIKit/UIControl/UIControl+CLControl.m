...
static char const * const cl_kSoundsKey = "cl_kSoundsKey";
...
- (void)cl_setSoundNamed:(NSString *)name forControlEvent:(UIControlEvents)controlEvent
{
    ...
    AVAudioPlayer *oldSound = [self cl_sounds][oldSoundKey];
    ...
    NSMutableDictionary *sounds = [self cl_sounds];
...
- (void)setCl_sounds:(NSMutableDictionary *)sounds
{
    objc_setAssociatedObject(self, cl_kSoundsKey, sounds, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSMutableDictionary *)cl_sounds
{
    NSMutableDictionary *sounds = objc_getAssociatedObject(self, cl_kSoundsKey);
    
    // If sounds is not yet created, create it.
    if (!sounds) {
        sounds = [[NSMutableDictionary alloc] initWithCapacity:2];
        // Save it for later.
        [self setCl_sounds:sounds];
    }
