...
+ (NSString *)cl_platformString{
    NSString *platform = [self cl_platform];
...
+ (NSUInteger)cl_getSysInfo:(uint)typeSpecifier
{
...
+ (NSUInteger)cl_cpuFrequency {
    return [self cl_getSysInfo:HW_CPU_FREQ];
}

+ (NSUInteger)cl_busFrequency {
    return [self cl_getSysInfo:HW_BUS_FREQ];
}

+ (NSUInteger)cl_ramSize {
    return [self cl_getSysInfo:HW_MEMSIZE];
}

+ (NSUInteger)cl_cpuNumber {
    return [self cl_getSysInfo:HW_NCPU];
}


+ (NSUInteger)cl_totalMemoryBytes
{
    return [self cl_getSysInfo:HW_PHYSMEM];
}
